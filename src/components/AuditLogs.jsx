...
import api from '../api'; // ያንተን የaxios instance ተጠቀም
...
const AuditLogs = () => {
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const response = await api.get('audit_logs_api.php');
  ...
  const deleteSelected = async () => {
    ...
    setError(null);
    try {
      const response = await api.delete('audit_logs_api.php', { data: { ids: selectedIds } });
      if (response.data?.success) {
