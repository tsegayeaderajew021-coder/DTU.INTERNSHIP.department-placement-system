...
import api from '../api';
import StudentAppeal from '../components/StudentAppeal';
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const resProfile = await api.get(`student_data_api.php?student_id=${encodeURIComponent(parsedUser.id)}&email=${encodeURIComponent(parsedUser.email || '')}`);
        ...
        const resPrefs = await api.get(`student_preferences.php?student_id=${parsedUser.id}`);
  ...
  useEffect(() => {
    ...
    const loadPlacementResult = async () => {
      ...
      try {
        const response = await api.get(`get_student_result.php?student_id=${encodeURIComponent(student.id)}`);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await api.post('student_preferences.php', {
